import { Lr as UserRejectedRequestError, Ta as getAddress, Ua as numberToHex, hi as formatUnits, kr as SwitchChainError, wa as checksumAddress } from "./secp256k1-DR4_-BNn.js";
import { it as parseUnits } from "./_esm-hbsOY9FW.js";
import { A as ChainNotConfiguredError, C as getConnection, D as disconnect, E as estimateGas, S as getConnections, T as getBalance, b as reconnect, c as createConnector, d as watchConnectors, f as watchConnections, g as ProviderNotFoundError, h as switchChain, k as connect, l as writeContract, m as waitForTransactionReceipt, p as watchConnection, s as injected, t as createConfig, u as watchPendingTransactions, v as signMessage, x as prepareTransactionRequest, y as sendTransaction } from "./createConfig-CGDjXFfS.js";
import { D as AssetController, H as CoreHelperUtil, J as proxy, M as OptionsController, U as StorageUtil, at as ErrorUtil, ct as PresetsUtil, g as getPreferredAccountType, ot as UserRejectedRequestError$1, p as getActiveCaipNetwork, q as subscribeKey, r as ChainController, v as ConnectorController } from "./ModalController-D0mbRgVR.js";
import { n as NetworkUtil, r as ConstantsUtil } from "./NetworkUtil-CSohOSHA.js";
import { c as SIWXUtil, i as HelpersUtil, l as AdapterBlueprint, n as CaipNetworksUtil, o as AlertController, r as ErrorUtil$1, s as WalletConnectConnector, t as W3mFrameProviderSingleton, u as WcHelpersUtil } from "./W3MFrameProviderSingleton-DTtNLwJy.js";
//#region node_modules/@reown/appkit-adapter-wagmi/dist/esm/src/connectors/AuthConnector.js
function authConnector(parameters) {
	...
	function parseChainId(chainId) {
		...
		if (!networks.some((n) => String(n.id) === String(chainId))) {
			...
			if (currentChainId && Number.isInteger(Number(currentChainId))) network = Number(currentChainId);
		}
		if (!network) throw new Error("ChainId not found in networks");
		return network;
	}
	function getProviderInstance() {
		if (!socialProvider) socialProvider = W3mFrameProviderSingleton.getInstance({
			...
			onTimeout: (reason) => {
				if (reason === "iframe_load_failed") AlertController.open(ErrorUtil$1.ALERT_ERRORS.IFRAME_LOAD_FAILED, "error");
				else if (reason === "iframe_request_timeout") AlertController.open(ErrorUtil$1.ALERT_ERRORS.IFRAME_REQUEST_TIMEOUT, "error");
				else if (reason === "unverified_domain") AlertController.open(ErrorUtil$1.ALERT_ERRORS.UNVERIFIED_DOMAIN, "error");
			},
			abortController: ErrorUtil$1.EmbeddedWalletAbortController,
	...
	async function connectSocial(options = {}) {
		...
		if (options.isReconnecting) {
			...
			if (!chainId) throw new Error("ChainId not found in provider");
		}
	...
	return createConnector((config) => ({
		...
		async connect(options = {}) {
			...
			if (!connectSocialPromise) connectSocialPromise = new Promise((resolve) => {
		...
		getAccounts() {
			if (!currentAccounts?.length) return Promise.resolve([]);
			config.emitter.emit("change", { accounts: currentAccounts });
			return Promise.resolve(currentAccounts);
		},
		async getProvider() {
			if (!this.provider) this.provider = W3mFrameProviderSingleton.getInstance({
				projectId: parameters.options.projectId,
				chainId: getActiveCaipNetwork()?.caipNetworkId,
				enableLogger: parameters.options.enableAuthLogger,
				abortController: ErrorUtil$1.EmbeddedWalletAbortController,
		...
		async getChainId() {
			const { chainId } = await (await this.getProvider()).getChainId();
			return parseChainId(chainId);
		},
		async isAuthorized() {
			const isActiveChainEvm = ChainController.state.activeChain === ConstantsUtil.CHAIN.EVM;
			if (ConstantsUtil.AUTH_CONNECTOR_SUPPORTED_CHAINS.some((chain) => ConnectorController.getConnectorId(chain) === ConstantsUtil.CONNECTOR_ID.AUTH) && !isActiveChainEvm) return false;
		...
		async switchChain({ chainId }) {
			try {
				...
				if (!chain) throw new SwitchChainError(/* @__PURE__ */ new Error("chain not found on connector."));
				const provider = await this.getProvider();
				const preferredAccountType = getPreferredAccountType("eip155");
				...
				if (error instanceof Error) throw new SwitchChainError(error);
				throw error;
			}
		},
...
//#endregion
//#region node_modules/@reown/appkit-adapter-wagmi/dist/esm/src/connectors/WalletConnectConnector.js
	...
	return createConnector((config) => ({
		...
		async connect({ ...rest } = {}) {
			try {
				...
				if (isRequestedChainSupported) currentChainId = requestChainId;
				else if (chains?.[0]) currentChainId = Number(chains[0].split(":")[1]);
				if (displayUri) {
		...
		async getAccounts() {
			const provider = await this.getProvider();
			if (!provider?.session?.namespaces) return [];
			const accounts = (provider?.session?.namespaces[ConstantsUtil.CHAIN.EVM]?.accounts)?.map((account) => account.split(":")[2]) ?? [];
			const accountsAdded = /* @__PURE__ */ new Set();
			return accounts.filter((account) => {
				const lowerCasedAccount = account?.toLowerCase();
				if (accountsAdded.has(lowerCasedAccount)) return false;
		...
		async getProvider({ chainId } = {}) {
			...
			if (chainId && currentChainId !== chainId && activeNamespace) {
				...
				const storedCaipNetwork = (activeNamespace ? ChainController.getCaipNetworks(activeNamespace) : [])?.find((n) => n.id === storedCaipNetworkId);
				if (storedCaipNetwork && storedCaipNetwork.chainNamespace === ConstantsUtil.CHAIN.EVM) await this.switchChain?.({ chainId: Number(storedCaipNetwork.id) });
			}
			return provider_;
		},
		async getChainId() {
			...
			const chain = (await this.getProvider()).session?.namespaces[ConstantsUtil.CHAIN.EVM]?.chains?.[0];
			return ChainController.getCaipNetworks().find((c) => c.id === chain)?.id;
		},
		async isAuthorized() {
			try {
		...
		async switchChain({ addEthereumChainParameter, chainId }) {
			...
			if (!chainToSwitch) throw new SwitchChainError(new ChainNotConfiguredError());
			try {
				await provider.request({
					method: "wallet_switchEthereumChain",
					params: [{ chainId: numberToHex(chainId) }]
				});
				if (chainToSwitch?.caipNetworkId) provider.setDefaultChain(chainToSwitch?.caipNetworkId);
				...
				const error = err;
				if (/(?:user rejected)/iu.test(error.message)) throw new UserRejectedRequestError(error);
				try {
					let blockExplorerUrls;
					if (addEthereumChainParameter?.blockExplorerUrls) blockExplorerUrls = addEthereumChainParameter.blockExplorerUrls;
					else blockExplorerUrls = chainToSwitch.blockExplorers?.default.url ? [chainToSwitch.blockExplorers?.default.url] : [];
		...
		getNamespaceChainsIds() {
			if (!provider_?.session?.namespaces) return [];
			return (provider_?.session?.namespaces[ConstantsUtil.CHAIN.EVM]?.accounts)?.map((account) => Number.parseInt(account.split(":")[1] ?? "")) ?? [];
		},
...
//#endregion
//#region node_modules/@reown/appkit-adapter-wagmi/dist/esm/src/utils/LimitterUtil.js
var state = proxy({ pendingTransactions: 0 });
var LimitterUtil = {
	state,
...
//#endregion
//#region node_modules/@reown/appkit-adapter-wagmi/dist/esm/src/utils/helpers.js
async function getSafeConnector(connectors) {
	if (CoreHelperUtil.isSafeApp()) {
		const { safe } = await import("./exports-uj2CsCwP.js");
		if (safe && !connectors.some((c) => c.type === "safe")) return safe();
	}
	return null;
}
async function getBaseAccountConnector(connectors) {
	try {
		const { baseAccount } = await import("./exports-uj2CsCwP.js");
		if (baseAccount && !connectors.some((c) => c.id === "baseAccount")) return baseAccount();
	} catch (error) {
...
async function getCoinbaseConnector(connectors) {
	try {
		const { coinbaseWallet } = await import("./exports-uj2CsCwP.js");
		if (coinbaseWallet && !connectors.some((c) => c.id === "coinbaseWallet")) return coinbaseWallet();
	} catch (error) {
...
//#endregion
//#region node_modules/@reown/appkit-adapter-wagmi/dist/esm/src/client.js
...
var WagmiAdapter = class extends AdapterBlueprint {
	...
	checkChainId() {
		...
		if (chainId) this.emit("switchNetwork", { chainId });
	}
	...
	createConfig(configParams) {
		...
		this.wagmiChains.forEach((element) => {
			...
			if (fromTransportProp) transports[element.id] = CaipNetworksUtil.extendWagmiTransports(element, configParams.projectId, fromTransportProp);
			else transports[element.id] = CaipNetworksUtil.getViemTransport(element, configParams.projectId, configParams.customRpcUrls?.[caipNetworkId]);
		});
	...
	setupWatchers() {
		watchConnections(this.wagmiConfig, { onChange: (connections) => {
			...
			this.addConnection(...connections.map((connection) => {
				...
				return {
					accounts: connection.accounts.map((account) => ({ address: this.toChecksummedAddress(account) })),
					caipNetwork,
		...
		watchConnection(this.wagmiConfig, { onChange: (accountData, prevAccountData) => {
			if (accountData.status === "disconnected" && prevAccountData.address) this.emit("disconnect");
			if (accountData?.chainId && accountData?.chainId !== prevAccountData?.chainId) this.emit("switchNetwork", { chainId: accountData.chainId });
	...
	async addThirdPartyConnectors() {
		...
		if (isBaseAccountEnabled !== false) {
			...
			if (baseAccountConnector) thirdPartyConnectors.push(baseAccountConnector);
		}
		if (isCoinbaseEnabled !== false) {
			const coinbaseConnector = await getCoinbaseConnector(this.wagmiConfig.connectors);
			if (coinbaseConnector) thirdPartyConnectors.push(coinbaseConnector);
		}
		const safeConnector = await getSafeConnector(this.wagmiConfig.connectors);
		if (safeConnector) thirdPartyConnectors.push(safeConnector);
	...
	addWagmiConnectors() {
		...
		if (OptionsController.state.enableInjected !== false) customConnectors.push(injected({ shimDisconnect: true }));
		...
		if (isEmailEnabled || isSocialsEnabled) customConnectors.push(authConnector({
			chains: this.wagmiChains,
			options: {
				projectId,
	...
	async handleAccountChanged({ address, chainId, connector }) {
		if (!this.namespace) throw new Error("WagmiAdapter:handleAccountChanged - namespace is required");
	...
	async signMessage(params) {
		try {
			return { signature: await signMessage(this.wagmiConfig, {
	...
	async writeContract(params) {
		...
		const wagmiChain = this.wagmiChains?.find((chain) => chain.id === chainId);
		return { hash: await writeContract(this.wagmiConfig, {
			chain: wagmiChain,
	...
	async estimateGas(params) {
		try {
			return { gas: await estimateGas(this.wagmiConfig, {
				account: params.address,
				to: params.to,
				data: params.data,
				type: "legacy"
	...
	async addWagmiConnector(connector) {
		if (!this.namespace) throw new Error("WagmiAdapter:addWagmiConnector - namespace is required");
		...
		if (isBasicInjected && isInjectedEnabled === false) return;
		if (isEip6963Connector && isEIP6963Enabled === false) return;
		if (connector.id === ConstantsUtil.CONNECTOR_ID.AUTH || connector.id === ConstantsUtil.CONNECTOR_ID.WALLET_CONNECT) return;
		let provider = void 0;
		if (connector.id !== ConstantsUtil.CONNECTOR_ID.BASE_ACCOUNT) provider = await connector.getProvider().catch(() => void 0);
	...
	async syncConnectors() {
		watchConnectors(this.wagmiConfig, { onChange: (connectors) => {
	...
	async syncConnections() {
		...
		await reconnect(this.wagmiConfig, { connectors: wagmiConnectors });
	}
	async syncConnection(params) {
		const { id, chainId } = params;
		const connection = getConnections(this.wagmiConfig).find((c) => c.connector.id === id);
		const provider = await this.getWagmiConnector(id)?.getProvider();
		if (CoreHelperUtil.isSafeApp() && id === ConstantsUtil.CONNECTOR_ID.SAFE && !connection?.accounts.length) {
	...
	async connectWalletConnect(chainId) {
		try {
			...
			if (!wagmiConnector) throw new Error("UniversalAdapter:connectWalletConnect - connector not found");
			const res = await connect(this.wagmiConfig, {
				connector: wagmiConnector,
				chainId: chainId ? Number(chainId) : void 0
			});
			if (res.chainId !== Number(chainId)) await switchChain(this.wagmiConfig, { chainId: res.chainId });
			return { clientId: await walletConnectConnector.provider.client.core.crypto.getClientId() };
		} catch (err) {
			if (err instanceof UserRejectedRequestError) throw new UserRejectedRequestError$1(err);
			if (ErrorUtil.isUserRejectedRequestError(err)) throw new UserRejectedRequestError$1(err);
			throw err;
		}
	}
	async connect(params) {
		try {
			...
			if (!connector) throw new Error("connectionControllerClient:connectExternal - connector is undefined");
			if (provider && info && connector.id === ConstantsUtil.CONNECTOR_ID.EIP6963) connector.setEip6963Wallet?.({
				provider,
				info
			});
			...
			if (connection) {
				...
				const sortedAccounts = [...connection.accounts].sort((a, b) => {
					if (HelpersUtil.isLowerCaseMatch(a, address)) return -1;
					if (HelpersUtil.isLowerCaseMatch(b, address)) return 1;
					return 0;
				});
	...
	async disconnect(params) {
		if (params.id) {
			const connector = this.getWagmiConnector(params.id);
			const connection = getConnections(this.wagmiConfig).find((c) => HelpersUtil.isLowerCaseMatch(c.connector.id, params.id));
			await disconnect(this.wagmiConfig, { connector });
			if (OptionsController.state.enableReconnect === false) this.deleteConnection(params.id);
	...
	async disconnectAll() {
		...
		const connections = await Promise.allSettled(wagmiConnections.map(async (connection) => {
			...
			if (connector) await disconnect(this.wagmiConfig, { connector });
			return connection;
		}));
		this.wagmiConfig.state.connections.clear();
		return { connections: connections.filter((connection) => connection.status === "fulfilled").map(({ value: connection }) => ({
			accounts: connection.accounts.map((account) => ({ address: this.toChecksummedAddress(account) })),
			connectorId: connection.connector.id
		})) };
	}
	async switchNetwork(params) {
		...
		if (!wagmiChain) throw new Error("connectionControllerClient:switchNetwork - wagmiChain is undefined");
	...
	async getCapabilities(params) {
		if (!this.wagmiConfig) throw new Error("connectionControllerClient:getCapabilities - wagmiConfig is undefined");
		const connection = getConnections(this.wagmiConfig)[0];
		const connector = connection ? this.getWagmiConnector(connection.connector.id) : null;
		if (!connector) throw new Error("connectionControllerClient:getCapabilities - connector is undefined");
		const provider = await connector.getProvider();
		if (!provider) throw new Error("connectionControllerClient:getCapabilities - provider is undefined");
	...
	async grantPermissions(params) {
		if (!this.wagmiConfig) throw new Error("connectionControllerClient:grantPermissions - wagmiConfig is undefined");
		const connection = getConnections(this.wagmiConfig)[0];
		const connector = connection ? this.getWagmiConnector(connection.connector.id) : null;
		if (!connector) throw new Error("connectionControllerClient:grantPermissions - connector is undefined");
		const provider = await connector.getProvider();
		if (!provider) throw new Error("connectionControllerClient:grantPermissions - provider is undefined");
	...
	async revokePermissions(params) {
		if (!this.wagmiConfig) throw new Error("connectionControllerClient:revokePermissions - wagmiConfig is undefined");
		const connection = getConnections(this.wagmiConfig)[0];
		const connector = connection ? this.getWagmiConnector(connection.connector.id) : null;
		if (!connector) throw new Error("connectionControllerClient:revokePermissions - connector is undefined");
		const provider = await connector.getProvider();
		if (!provider) throw new Error("connectionControllerClient:revokePermissions - provider is undefined");
		return provider.request({
	...
	async walletGetAssets(params) {
		if (!this.wagmiConfig) throw new Error("connectionControllerClient:walletGetAssets - wagmiConfig is undefined");
		const connection = getConnections(this.wagmiConfig)[0];
		const connector = connection ? this.getWagmiConnector(connection.connector.id) : null;
		if (!connector) throw new Error("connectionControllerClient:walletGetAssets - connector is undefined");
		const provider = await connector.getProvider();
		if (!provider) throw new Error("connectionControllerClient:walletGetAssets - provider is undefined");
		return provider.request({
	...
	setAuthProvider(authProvider) {
		if (!this.namespace) throw new Error("WagmiAdapter:setAuthProvider - namespace is required");
		this.addConnector({
			id: ConstantsUtil.CONNECTOR_ID.AUTH,
			type: "AUTH",
			name: ConstantsUtil.CONNECTOR_NAMES.AUTH,
	...
	async setUniversalProvider(universalProvider) {
		universalProvider.on("connect", () => {
			...
			if (connector && !connections.find((c) => c.connector.id === connector.id)) {
				if (ChainController.state.activeChain === "eip155") return;
				reconnect(this.wagmiConfig, { connectors: [connector] });
			}
		});
		const walletConnectConnector = walletConnect({ universalProvider });
...
//#endregion
export { WagmiAdapter, authConnector };
